...
import LignesFacture from '../factures/LignesFacture';
...
class FactureFromDevis extends Component {
  constructor(props) {
    super(props);
  ...
  handleEnregitrer = (event) => {
    ...
    const { clearFactureState, history, createFacture2 } = this.props;
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const devis = nextProps.devis ? nextProps.devis : this.state.facture;
    ...
    facture.notesFacture = devis.notesDevis;
    facture.conditionFacture = devis.conditionDevis;
    facture.client = { ...devis.client };
    //facture.lignesFacture = JSON.parse(JSON.stringify(devis.lignesDevis));//cette version ne marche pas
    facture.lignesFacture = nextProps.devis
      ? JSON.parse(JSON.stringify(nextProps.devis.lignesDevis))
      : [
          {
            produit: '',
            quantite: '',
    ...
    facture.nomEntreprise = devis.nomEntreprise;
    this.setState({ facture });
  }

  //version ancienne qui marche
  /*   componentWillReceiveProps(nextProps) {
    console.log(nextProps);
    const nbrFactureOfThisYear = nextProps.factures
      ? nextProps.factures.filter(
    ...
    let facture = { ...this.state.facture };
    facture.dateFacture = new Date();
    facture.dateEcheanceFacture = new Date();
    facture.notesFacture = nextProps.devis ? nextProps.devis.notesDevis : '';
    facture.conditionFacture = nextProps.devis
      ? nextProps.devis.conditionDevis
      : '';
    facture.client = nextProps.devis
      ? { ...nextProps.devis.client }
      : {
          nomClient: '',
          adressClient: '',
          paysClient: '',
    ...
    facture.devisId = nextProps.devis ? nextProps.devis.id : null;
    this.setState({ facture });
  } */
  ...
  render() {
    const { facture, showSpinner, fichiersDeLaFacture } = this.state;
    ...
    return (
      <div>
        ...
        <form onSubmit={this.handleEnregitrer}>
          ...
          <div className="row">
            <h3 className="ml-3 float-left">
              Facture (à partir du devis n°{devis ? devis.numDevis : null})
            </h3>
          </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    devis: state.firestore.ordered.devis && state.firestore.ordered.devis[0],
    companyData:
